...
Model::Control RosbotNOPController::calcNOPControl(const Model::State& currState)
{
	...
	delta.y = (m_goal.x - currState.x) * (currState.y - m_prevState.y) - (m_goal.y - currState.y) * (currState.x - m_prevState.x);

	std::cout<<"m_goal.x "<<m_goal.x<<" m_goal.y "<<m_goal.y<<std::endl;
	...
	float targetYaw = atan2(m_goal.y - currState.y, m_goal.x - currState.x);
	delta.yaw = targetYaw - currState.yaw;
